...
**WebFinger** (RFC 7033) is the discovery mechanism that makes [federation](federation) work. Given a handle like `@user@domain.org`, WebFinger tells you where to find that entity's full identity document.
...
The response is a JSON Resource Descriptor (JRD) pointing to the [ActivityPub](activitypub) actor document:
...
## Related Concepts
- [ActivityPub](activitypub) — the protocol that WebFinger enables discovery for
- [Federation](federation) — the network model that requires cross-server discovery
