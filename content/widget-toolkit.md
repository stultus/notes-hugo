---
title: Widget Toolkit
date: 2024-11-04
draft: false
tags: ["computer-science", "ui-ux"]
status: "seeding"
summary: "A library for building graphical user interfaces."
---

A **Widget Toolkit** (or GUI toolkit) is a library or a collection of libraries containing a set of graphical control elements (called widgets) used to construct the graphical user interface (GUI) of programs.

## Related Concepts
- [Application Layout Engine](/notes/application_layout_engine/) - Uses widget toolkits for UI layout
- [Layout Engine](/notes/layout-engine/) - General rendering concept
- [User Interface Markup Language](/notes/user-interface-markup-language/) - Declarative UI definition

## Resources
- [Wikipedia Link](https://en.wikipedia.org/wiki/Widget_toolkit)

